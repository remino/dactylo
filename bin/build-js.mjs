...
await buildLibrary({
	entry: resolve(root, 'src/lib/dactylo.ts'),
	fileName: format => (format === 'es' ? 'dactylo.mjs' : 'dactylo.cjs'),
	formats: ['es', 'cjs'],
})
...
await buildLibrary({
	...
	fileName: format =>
		format === 'es' ? 'dactylo-auto.mjs' : 'dactylo-auto.cjs',
	formats: ['es', 'cjs'],
})

await buildLibrary({
	entry: resolve(root, 'src/lib/auto.ts'),
	fileName: () => 'dactylo.umd.js',
	formats: ['umd'],
	minify: true,
	name: 'dactylo',
})
